...
import com.example.applicationhotel.exception.InternalServerExceptoion;
...
public class RoomServiceImpl implements IRoomService{
    ...
    public void deleteRoom(Long roomId) {
        Optional<Room> theRooom = roomRepository.findById(roomId);
        if(theRooom.isPresent()){
         ...
         if(photoBytes != null && photoBytes.length > 0) {
             try{
                 room.setPhoto(new SerialBlob(photoBytes));
                    ...
                    throw  new InternalServerExceptoion("Error updating room");
             }
         }
         return roomRepository.save(room);
    }
